...
func (h *Handler) RegisterRoutes(ws *restful.WebService) {
	ws.Route(ws.POST("/example").To(WrapBody[User](h.Example)).
		Doc("创建示例").
		Reads(User{}).
		Returns(200, "创建成功", &User{}))
}

func (h *Handler) Example(r *restful.Request, w *restful.Response, req User) (httpx.Result, error) {
	data := &User{
		Username: "张三",
		Email:    "123456@qq.com",
		Password: "123456",
